...
from .parser import RichTextParser, TextType
from .text_box import VirtualTextBox, TextObject, ImageObject, RenderItem
...
class RichTextRenderer:
    ...
    def draw_complex_text(self, text: str, polygon_vertices: List[Tuple[int, int]],
                          ...
                          layout_only: bool = False) -> list[RenderItem]:
        """
        ...
        """
        if draw_debug_frame and not layout_only:
        ...
        # 旧的静默底图阶段仍会绘制 <flavor quote> 辅助线和 <hr> 线段。
        # layout_only 只跳过文字绘制，但必须保留这些非文字线段，才能像素级复刻旧底图。
    ...
    def draw_line(self, text: str, position: Tuple[int, int],
                  ...
                  max_length: Optional[int] = None, debug_draw_range: bool = False,
                  layout_only: bool = False) -> list[RenderItem]:
        """
        ...
        use_enhanced = (not self.font_manager.silence and not layout_only) and self._use_enhanced_draw(options)
        text_opacity = self._sanitize_opacity(options.opacity)
        base_effects = self._prepare_effects(options.effects) if use_enhanced else []
        ...
        if not self.font_manager.silence or layout_only:
        ...
        if options.has_underline and not layout_only:
            if vertical:
                # 垂直模式的下划线（实际是侧边线）
